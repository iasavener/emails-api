...
const MongoService = {
  ...
  insertEmails: async (emailData) => {
    try {
      const encryptedEmails = emailData.map((email) =>
        encryptFields(email, ["from", "subject", "body"])
      );
  ...
  getEmails: async (employee_id, page = 1, limit = 10, project_id) => {
    ...
    if (project_id) {
      queryFilter.project_id = project_id;
    }

  
    const totalItems = await Email.countDocuments(queryFilter);

 
    const emails = await Email.find(queryFilter).skip(skip).limit(limit);

 
    const decryptedEmails = emails.map((email) =>
      decryptFields(email.toObject(), ["from", "subject", "body"])
    );
  ...
  updateProjectAssigned: async (employee_id, uid, project_id, projectName) => {
    return await Email.updateOne(
      { uid, employee_id },
      { $set: { project_id, project_name: projectName } }
    );
  },

  setEmails: async (employee_id, emails) => {
    return await Email.insertMany(
      emails.map((email) => ({ ...email, employee_id })),
      { ordered: false }
    );
  },
  ...
  setReadEmails: async (employee_id, uids) => {
    return await Email.updateMany(
      { employee_id, read: false, uid: { $nin: uids } },
      { $set: { read: true } }
    );
  },
  ...
  getLastExecution: async (employee_id) => {
    return await Execution.findOne({ employee_id, deleted: false }).sort({
      date: -1,
    });
  },
  ...
  updateClassification: async (uid, project_id, projectName) => {
    ...
    return await Email.updateOne(
      {
        uid,
        $or: [{ discard: { $exists: false } }, { discard: false }],
        $or: [{ saved: { $exists: false } }, { saved: false }],
      },
      {
        $set: {
          project_id,
